...
- **Active framework version**: AMAS v3.0.4 (active, in development in this repository); latest published release v3.0.0 — see the README version-positioning note
...
- **Builder surface (Claude Code)**: executes Architect handoffs; honors `core.md` §8.2 and §8.3 disciplines; hands back at named hand-back points; never merges.
- **Both**: escalate to owner on scope drift, blocking questions, or any condition the handoff did not anticipate.
- **Both**: claimed actions must be true. Do not narrate actions not actually performed. If a `core.md` §23.6 sweep cannot complete within session budget, hand back per `core.md` §23.6.5 rather than handing partial work to the next surface.
