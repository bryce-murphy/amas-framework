...
**§3.9. Merge.** The human merges. If the repo is single-contributor, the merge uses GitHub's bypass mechanism with explicit acknowledgment in the squash-commit message or a pinned PR comment per `core.md` §10.5 (forthcoming at Part C+). This is supported, not a workflow violation. The first bypass on a repo warrants a dedicated ADR documenting the pattern.
...
- **Bypass rule:** Single-contributor repos bypass with acknowledgment in the squash-commit message, PR comment, or PR template `Bypass used` field. First bypass gets an ADR.
