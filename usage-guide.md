...
**§3.9. Merge.** The human merges. If the repo is single-contributor, the merge uses GitHub's bypass mechanism per `core.md` §10.5. This is supported, not a workflow violation. Each bypass invocation is logged automatically by GitHub and is itself the acknowledgment of full operator responsibility for the merge state; **no additional artifact is required at single-cycle scope** per `github-reference.md` §3.2 (an explicit note in the squash-commit message or a pinned PR comment is optional, not required). Sustained-multi-contributor projects should document the bypass posture in an ADR.
...
- **Bypass rule:** Single-contributor repos bypass per `core.md` §10.5 / `github-reference.md` §3.2 — GitHub's automatic bypass log is the acknowledgment; no additional artifact is required at single-cycle scope (an explicit note in the squash-commit message, a pinned PR comment, or a PR-template `Bypass used` field is optional). Sustained-multi-contributor projects document bypass posture in an ADR.
...
- **Executor pre-flight per §8.2:** Before branch creation and before any repo-writing, remote-visible, or destructive action, verify base branch, working-tree state, file-existence assumptions, CI-format compliance, numerical identifier assumptions, base-branch freshness. Stop-and-report on failure.
