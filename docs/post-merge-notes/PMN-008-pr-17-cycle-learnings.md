...
Six monitoring items carry forward (five from TASK-0017 spec §1.3 + (t) demotion). Two new candidates registered: (u) v2.14.1-canonical-vs-repo-convention divergence (from PR-17 Codex post-PR pass 1); (v) self-instantiation-via-failure-and-correction at the surface canonicalizing the discipline (from PMN-008 authoring cycle itself — see §1.1 below).
...
**PMN-008 authoring honesty record**: Architect §23.6 self-review on PMN-008 was single-iteration "clean enough for presentation" — insufficient. Builder PR-19 self-review surfaced **eight substantive defects across three sweep passes** (seven pre-authoring at step-1 pre-flight + one post-authoring at step-6 (j) sweep) before commit:
...
2. **§4.2 cited [github-reference.md](github-reference.md) §3.5 placement target without verification** (§3 has only §3.1/§3.2/§3.3 in HEAD; no §3.5). Same defect class as TASK-0017 spec iteration-2 Defect E (Architect-asserted-without-verification — failed to grep actual canonical structure). (i.5) self-application failure on the very discipline PMN-008 canonicalizes. Path-(a) routed: drop specific anchor; defer placement adjudication to TASK-0019 Phase 1 scoping.
...
**Recursive empirical confirmation of (r) fifth-surface canonical refinement** at strongest possible strength: the discipline PMN-008 canonicalizes self-applied at the very next PR cycle (PR-19 PMN-008 authoring) to catch defects that escaped Architect §23.6 single-iteration sweep on PMN-008 itself. The eight-defect tally splits cleanly across three distinct sweep passes (§-content at Pass 1 pre-flight; convention-inference at Pass 2 pre-flight per (i.5) discipline; (j) §-citation all-instances sweep at Pass 3 Builder step-6 self-review post-authoring) — each pass catches defect classes that align precisely with the surface-coverage table in §3.1 below. Strong empirical reinforcement that multi-surface mitigation per PMN-006 §1.1 / §3.2 framing is structurally load-bearing — single Architect-surface fixed-point is insufficient even with the (r) and (i.5) disciplines canonicalized in the document being authored.
...
### §5.2. (q') clean-first-pass conditional on PR scope — two-data-point divergence

**Observation**: PR-15 Codex post-PR = clean first pass (0 findings; (q) initial candidate). PR-17 Codex post-PR pass 1 = 2 substantive content findings. The divergence correlates with PR content-surface size: PR-15 was a smaller canonical-text extension; PR-17 was a full canonical-law-trio-member authoring (~391 lines of substantive canonical content).

**Refinement candidate**: (q') = "clean-first-pass Codex post-PR shape is conditional on PR scope (small canonical-text extension ≈ clean; full canonical-law-trio-member authoring ≈ findings expected)."

**Promotion trigger**: 2 more data points confirming scope-conditional pattern. Watch: usage-guide.md (full canonical-law-trio-member; ~comparable scope to PR-17) + process-templates batch (multi-file migration; different scope shape).
...
**Observation**: PMN-008 canonicalizes (r) fifth-surface Builder step-6 self-review and (i.5) convention-inference verification disciplines. Builder PR-19 self-review caught eight defects in PMN-008 itself (see §1.1) across three sweep passes — three §-content defects (Pass 1 pre-flight) plus four frontmatter/structure defects (Pass 2 pre-flight per (i.5) sub-extension) plus one §-citation residual (Pass 3 Builder step-6 (j) sweep post-authoring). The defects arose precisely from Architect failure to apply (r) and (i.5) and (j) to PMN-008's own authoring; the disciplines self-instantiate negatively at the Architect surface and positively at the Builder surface within the same authoring cycle, and at the canonical step-6 surface specified in §3.1 (rather than step-1 pre-flight) for the (j) §-citation sweep catch.
...
**TASK-0019 spec (usage-guide.md authoring)** carries forward PMN-008's two canonical refinements as defaults:

- Five-surface review pipeline per (r) as canonical default in §5 of spec.
- (i.5) + Class A/B/C version-marker classification + (j) all-instances sweep completeness in pre-authoring batch per §4.2; (i.5) PMN-file-shape sub-extension applies at any PMN authoring within the cycle.
...
**Branch regex reconciliation cycle** (deferred from PR-17 path-(β)): anticipated at TASK-0019 Phase 1 scoping or following. Architect adjudicates: (a) read CLAUDE.md actual `task-NNNN` form; (b) determine canonical source for v3 branch convention (CLAUDE.md vs v2.14.1 §6.1 vs new ADR); (c) update [github-reference.md](github-reference.md) §2.2/§2.3 via surgical fix if canonical source is CLAUDE.md form; otherwise record canonical-source-is-v2.14.1 as ADR.
