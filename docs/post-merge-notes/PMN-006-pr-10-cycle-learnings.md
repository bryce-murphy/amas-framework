...
Ten defects originated in or were surfaced during this cycle:
...
10. **PMN-006 §3.2 (h.3) canonical-form articulation was itself defective — multi-key ordering correctness sub-aspect**. The (h.3) sub-shape canonicalized in this PMN cycle initially named "`>=` plus last-seen-ID tie-breaker" as the canonical filter form. Codex post-PR pass 2 on PR-11 (commit `421fda1`) surfaced that this naive form drops valid new emissions whose timestamp exceeds the boundary but whose id is below the last-seen-id — possible when ids do not strictly track submitted_at/created_at (e.g., a long-drafted review id N1 submitted at t2 after a faster-drafted review id N2 > N1 submitted at t1). The correct form is the **lexicographic tie-break**: `(timestamp > last-known) OR (timestamp == last-known AND id > last-seen-id)` — gates the id comparison only inside the same-timestamp tie clause; cross-timestamp emissions pass through the `timestamp > last-known` clause without id constraint. Path-(a) revised in this same PR-11 cycle: core.md §8.1.1.1 lines 45 and 49 corrected to lexicographic form on both endpoints; surrounding prose (line 52) updated to explain the multi-key correctness rationale; PMN-006 §3.2 (h.3) sub-shape description updated to specify lexicographic as canonical and call out the naive form as defective. **Recursive-self-instantiation case at one further iteration**: the cycle that canonicalizes the (h) verification-command operational correctness discipline produces a canonical text that itself fails (h.3); subsequent iteration corrects the canonical text within the same cycle that introduces it. Surfaced via Codex cloud post-PR review (`chatgpt-codex-connector[bot]`) on `421fda1` at 2026-05-03T00:43:02Z. Empirically demonstrates that multi-stage review pipeline (§23.6 self-review + Builder step-10 + Codex desktop pre-commit + Codex cloud post-PR) is load-bearing for canonical-text correctness even when the cycle's purpose is canonicalizing the discipline that the canonical-text-defect itself instantiates.

The framework operated as designed for defects 1, 2, and 10. Multi-stage review (§23.6 self-review + Builder step-10 + Codex pre-commit + Codex post-PR auto-fire + Codex post-PR manual `@codex review`) is load-bearing, not redundant. Pass 5 of PR-10 caught what 4 prior review stages missed; pass 2 of PR-11 cloud post-PR review caught what Architect-side §23.6 self-review (5 iterations) + Builder pre-flight (3 passes) + Codex desktop pre-commit (1 pass) + Codex cloud post-PR pass 1 (1 pass) all missed. Defects 6, 7, 8 were caught by Builder pre-flight passes 1, 2, 3 respectively before any commits; defect 9 was caught by Builder at step 8 execution time (post-pre-flight); defect 10 was caught by Codex cloud post-PR pass 2 at PR-11 close (post-step-14 stop-and-show). The framework's session-handoff verification step (spec §5 step 1) is structurally load-bearing for defects of class (i.1) and (i.2) (tracked-file presence; filesystem path); defect 9 surfaces a (i.3) content-pattern sub-shape that requires verification at the time the assertion is acted on (step 8 execution), not at session start when the assertion is read. Defect 10 surfaces a recursive-self-instantiation pattern at one further iteration — the cycle that canonicalizes (h) produces canonical text that itself fails (h.3); the multi-stage review pipeline catches it before merge. This is a refinement to the (i) discipline canonical placement: (i.3) verification is properly distributed across the cycle's execution, not concentrated at session start; further, canonical-text defects in newly-canonicalized disciplines may surface only at the post-PR-cloud-review stage, requiring the full review pipeline including post-PR cloud review for canonical-text correctness assurance. Defects 3, 4, 5, 6, 7, 8, 9, 10 are §23.6 sweep-set gaps that the (g)/(h)/(i) discipline split (§3 and §7 below) addresses canonically — defect 10 specifically motivates a refinement to (h.3) sub-shape canonical-form articulation within the same PMN cycle that canonicalizes (h).
...
- **(h.3) Filter-boundary completeness**. Strict-comparator filters on coarse-granularity inputs drop boundary-coincident emissions; correct form is the **lexicographic tie-break**: `(timestamp > last-known) OR (timestamp == last-known AND id > last-seen-id)`. The naive `(timestamp >= last-known AND id > last-seen-id)` form is INCORRECT — it requires `id > last-seen-id` for all results regardless of timestamp, which drops valid new emissions whose timestamp exceeds the boundary but whose id is below the last-seen-id (possible when ids do not strictly track timestamps). Lexicographic form gates the id comparison only inside the same-timestamp tie clause. Caught by Codex post-PR pass 5 of PR-10 (strict-`>` defect — same-second-tie-completeness aspect) and Codex post-PR pass 2 of PR-11 (naive-`>=`-with-unconditional-id-gate defect — multi-key ordering correctness aspect; per §1.1 defect 10). Drives the §8.1.1.1 polling-commands canonical-text correction in PR-11 (applied symmetrically to both `submitted_at` and `created_at` endpoints).
...
1 cycle's empirical evidence (this cycle's TASK-0011 spec + canonical-text PR). 10 distinct sub-defect surfacings within that single cycle (4 pre-flight pass-1 + 1 pre-flight pass-2 + 3 pre-flight pass-3 residuals + 1 step-8-execution-time defect 9 + 1 Codex-cloud-post-PR-pass-2 defect 10). Above the typical 1-cycle/1-data-point threshold for single-cycle observation but below the ~3-cycle threshold for canonical-refinement promotion. (i) is canonicalized at PMN-006 because the discipline addition is small and the cycle introducing (g)/(h) is the right context for grouping; future cycles refine sub-shape taxonomy if additional sub-defect classes surface. The defect-9 surfacing-at-step-8-execution-time data point is a refinement candidate to the (i) discipline canonical-placement: (i.3) content-pattern verification timing should be distributed across cycle execution rather than concentrated at session-start pre-flight. The defect-10 surfacing-at-Codex-cloud-post-PR-pass-2 data point is a refinement candidate to the multi-stage review pipeline composition: canonical-text correctness on newly-canonicalized disciplines requires the full pipeline including post-PR cloud review, not just Architect §23.6 + Builder pre-flight + Codex desktop pre-commit.
