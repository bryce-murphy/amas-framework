...
- **Observe-then-canonicalize status**: strong first-evidence (two positive in-cycle instances + one negative/recurrence instance); continue observing toward the 3+ bar.
...
- **§23.6.5-refinement candidate (light; observe-then-canonicalize, NOT a this-cycle change)**: the handoff §Cumulative-diff-stats **full-staged-total** figure is **self-volatile** — every absorption push edits the handoff (and review-context/PMN), changing the total the figure records, so a gate-current full-total recurs stale at each commit and needs a fixed-point re-derivation each push (observed at pass-3: editing L44 to record the total is self-referential). Candidate refinement: reframe the full-staged-total line as a **commit-pinned snapshot** ("as of `<commit>`", append-only / §23.6.5-suppression-protected) rather than a gate-current pointer — keeping only the **stable canonical+operational figure** (byte-unchanged across artifact-only fix-ups) as the gate-current number. Registration only; defer canonicalization to a future §23.6.5 amendment cycle.
