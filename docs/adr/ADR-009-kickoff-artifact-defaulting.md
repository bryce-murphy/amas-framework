...
Worked instances at v3.0: `doc_mcp_mechanism` carries the schema-valid default `manual canonical-doc reference + owner-verified retrieval` (richer options forthcoming v3.2); the TASK-0000 bootstrap handoff uses the `PR-0` schema-valid sentinel (satisfies the strict `PR-(\d+)` form), `linked_predecessor: none`, `spec_source: ADR-000`; the PR-template §8.2/§8.3 items are marked non-blocking (`N/A until Part C.2 (TASK-0048)`) rather than asserted as false attestations.
