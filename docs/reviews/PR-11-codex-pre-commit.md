...
10. **Frontmatter-vs-body reconciliation clean per (b.3) extended PMN-003 (a) refined.** PMN-006 has no YAML frontmatter (PMN bodies use markdown title + Status block per §18 canonical form), so frontmatter-vs-body reconciliation primarily applies to TASK-0011 handoff (which has frontmatter `task_id`, `linked_predecessor`, `linked_pr`, etc.). Verifiable at pre-commit:
    - PMN-006: title line + first ## §1 reference internal consistency — title says "PR-10 cycle learnings"; §1 says "PR-10 (TASK-0010) authored core.md Part A". Verify: `head -3 docs/post-merge-notes/PMN-006-pr-10-cycle-learnings.md | grep -c "PR-10"` returns at least 2.
...
Action: Verify the README.md diff against main is limited to the PMN file table (one new PR-11 row). Any modification to the Package layout table or any other location is a Blocking finding (out of scope per PMN-006 spec §2 deliverable 5 + ADR-003 §Consequences).
