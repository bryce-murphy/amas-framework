...
- **(h) Verification-command operational correctness** (NEW this PMN, applied here): for each claim block, verify `--paginate` is present where reachable (h.1); verify the named command's output domain covers the claim's verification need (h.2); verify timestamp filters use lexicographic OR form `(timestamp > last-known) OR (timestamp == last-known AND id > last-seen-id)` per (h.3) corrected canonical form (NOT the naive `>=`-plus-unconditional-id-gate form, which was the originally-canonicalized but defective form per PMN-006 §1.1 defect 10 and §3.2 (h.3) refined). The (h.2) and (h.3) sub-shapes are the primary motivators for this PR's `core.md` corrections (claims 7 and 8 above).
