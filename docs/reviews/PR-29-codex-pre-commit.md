...
15. **Cumulative-diff-stats per (e.1) sub-rule**: per-file numstat sums reconcile to `git diff --shortstat origin/main` total exactly; no `~`-prefixed approximate counts in any artifact. Final landed post step-16-tris re-iteration per TASK-0023 cycle-close N4 staged-tree convention applied at each iteration:
    - bash: `git diff --numstat origin/main` returns 4 rows (full per-file numstat at final branch state post-step-16-tris): `382	3	usage-guide.md` + `2	2	README.md` + `202	0	docs/handoffs/TASK-0024-usage-guide-substantive-fill.md` + `181	0	docs/reviews/PR-29-codex-pre-commit.md`. Iteration history: step-9 self-review iteration baseline 4/661/5 (handoff 171/0 + review-context 106/0 pre-absorption); step-11 Codex pre-commit absorption iteration 4/760/5 (handoff 195/0 + review-context 181/0); step-16 fix-up commit absorption iteration 4/765/5 (handoff 200/0 + N12-N16 ledger); step-16-bis re-iteration 4/766/5 (handoff 201/0 + N17 phantom-SHA + Path-(a)-comprehensive line-count-preserving substitutions); step-16-tris re-iteration 4/767/5 (handoff 202/0 + N18 intra-doc internal-consistency + §10.22 cross-reference Path-(a)-narrow-A substitution).
    - bash: `git diff --shortstat origin/main` returns `4 files changed, 767 insertions(+), 5 deletions(-)` (final landed at step-16-tris re-iteration commit).
    - Sum-stability check at step-16-tris final landed: insertion-column sum 382+2+202+181 = 767 ✓; deletion-column sum 3+2+0+0 = 5 ✓. All cited counts in this review-context (claims 1, 2, 4, 7, 8, 14, 15) re-derived from current branch state per (e.1) sub-rule; no approximate (`~`-prefixed) counts.
