...
15. **Cumulative-diff-stats per (e.1) sub-rule**: per-file numstat sums reconcile to `git diff --shortstat origin/main` total exactly; no `~`-prefixed approximate counts in any artifact. Final landed post step-16-quater re-iteration per TASK-0023 cycle-close N4 staged-tree convention applied at each iteration:
    - bash: `git diff --numstat origin/main` returns 4 rows (full per-file numstat at final branch state post-step-16-quater): `382	3	usage-guide.md` + `2	2	README.md` + `203	0	docs/handoffs/TASK-0024-usage-guide-substantive-fill.md` + `181	0	docs/reviews/PR-29-codex-pre-commit.md`. Iteration history: step-9 baseline 4/661/5 (handoff 171/0 + review-context 106/0); step-11 Codex pre-commit absorption 4/760/5 (handoff 195/0 + review-context 181/0); step-16 fix-up 4/765/5 (handoff 200/0 + N12-N16 ledger); step-16-bis re-iteration 4/766/5 (handoff 201/0 + N17 phantom-SHA); step-16-tris re-iteration 4/767/5 (handoff 202/0 + N18 intra-doc internal-consistency + §10.22 cross-reference); step-16-quater re-iteration 4/768/5 (handoff 203/0 + N19 Architect-spec-authoring-origin meta-pattern + §3.5/§3.9/§13 path-(a) substitutions).
    - bash: `git diff --shortstat origin/main` returns `4 files changed, 768 insertions(+), 5 deletions(-)` (final landed at step-16-quater re-iteration commit).
    - Sum-stability check at step-16-quater final landed: insertion-column sum 382+2+203+181 = 768 ✓; deletion-column sum 3+2+0+0 = 5 ✓. All cited counts in this review-context (claims 1, 2, 4, 7, 8, 14, 15) re-derived from current branch state per (e.1) sub-rule; no approximate (`~`-prefixed) counts.
