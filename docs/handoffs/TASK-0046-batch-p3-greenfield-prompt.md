...
Cleared at cycle-close hand-back per `core.md` §24.3.1 (XXVI) two-gate five-point check. Substantive cycle (PR #75): Gate A (staged-tree, step-9 + step-11 re-application) + Gate B (post-push, step-13). Post-close fix-chore (PR #77 + #78): its own Gate A + Codex sandbox execution-validation. Each point grounded in actual cycle events:

1. **Three-endpoint poll (§8.1.1.1)** — Codex pre-commit pass-1 (2 substantive, 0 blocking) + post-PR pass-1 (1 P2 + 2 suggestions + 1 phantom-claim) reconciled across formal-review / issue-comment / line-comment endpoints; absorptions recorded at §6 / §8 + PR-75 review-context.
2. **Tip-state proof** — Gate A staged-tree parity 8 files / 513 ins / 13 del with (XVII) bidirectional sum-stability at all three axes (snapshot at step-13 Gate B; volatile — see §Cumulative-diff-stats); Gate B branch tip `79d86f8` pre-squash → squash `32c4afa` on main (= #75 `mergeCommit`, verified real).
3. **File-content audit** — greenfield 7-obligation coverage + co-shipped surfaces (§18.3 M-A7, Class A v2.42 ×4, README L30 + Prompts row, usage-guide L22/L370) verified against spec §2/§4 at Gate A; post-merge §8.1.1.2 audit then caught the PMN-001 (k) Action over-reach (greenfield wrongly `resolved`), remediated at PR #77 / #78 (ledger (XI)).
4. **Phantom-action audit** — Codex post-PR claimed commit `2b5cfd1` + `make_pr` PR verified non-existent (ledger (IX)); the Action's own outputs verified against repo state, not the fire signal (verify-the-Action; ledger (XI)).
5. **Comment-content claim verification (§8.1.1.2)** — P2 (handoff §Current-state status staleness) verified + absorbed path-(a) at step-13; fix-chore F-5 (yml generated-PR-body stale-scope text) verified + absorbed path-(a) at PR #77.

Cycle-close canonical markers (the sole close markers, not body prose): handoff `linked_pr` = `PR-75 (squash SHA 32c4afa)`, handoff `status: resolved`, review-context `status: recorded` — all action-filled on main. Corrected-main tip at this reconciliation: `dcfc7ea` (post fix-chore PR #77 + #78).
...
- **(VI)** PR-number anticipation verification at step-11 — PR-75 anticipated across 5 surfaces; phantom-correct if divergent. Verified at step-12: actual = PR-75, matching anticipation across all 5 surfaces (greenfield `filled_by`, README Prompts row, §18.3 M-A7 enumeration, handoff `pr`/`linked_pr`, review-context filename); no phantom-correction needed.
- **(VII)** §24.5 multi-surface review pipeline empirical record at first prompt-class cycle. Pre-commit pass-1 (2 substantive, 0 blocking — F-2 path-(a) greenfield hardening + root-gap (VIII); F-1 gate-current refresh) + post-PR pass-1 (P2 path-(a) gate-current refresh + 2 suggestions + §8.1.1.2 phantom-claim (IX)). Pipeline effective at first prompt-class cycle: substantive prompt content clean; residuals were documentation / gate-current-status class. Detail at §6 / §8 + PR-75 review-context.
...
- **(XI)** Post-close fix-chore — PMN-001 (k) Action status-scoping over-reach (PRs #77 + #78). The `linked-pr-fix-up` Action's `apply_substitutions` applied status transitions (`active->resolved`, `drafted->recorded`) to the frontmatter of every changed `.md/.yml/.yaml` with no path scoping, and wrongly flipped `prompts/greenfield.md` `active->resolved` at the #76 auto-chore (first cycle to ship a `status: active` prompt artifact, so first to expose it). Caught by applying §8.1.1.2 phantom/action-verification to the Action's own outputs (verify-the-Action extension), not the fire signal. Fix (PR #77, squash `5125138`): path-scope transitions — `active->resolved` bound to `docs/handoffs/`, `drafted->recorded` to `docs/reviews/` + `docs/post-merge-notes/`, `prompts/`+`templates/` excluded; `PLACEHOLDER_PATTERN` (linked_pr SHA backfill) preserved unchanged; `greenfield.md` restored to `active`; yml header comment + generated-PR-body text corrected. (a) Guard-skip-on-self-fix: #77's branch was recursion-guard-matched (`chore/task-0046-pmn-001-k`) so the Action skipped its own fix-merge — avoiding debuting a freshly-changed auto-firing workflow on the merge that changes it; first live run deferred to TASK-0047's chore-fix-up. (b) Codex execution-validation: the locally-un-runnable script (Windows, no `python3`) was compile- and trace-validated in Codex's sandbox (8 scoping cases PASS) — the execution layer the Builder could not provide. (c) Backfill (PR #78, squash `dcfc7ea`): manual PMN-018 `linked_pr` backfill (guard-skip meant no auto-fill), via an isolated 1-line PR; the non-guard #78 merge triggered the now-scoped Action, which correctly NO-OP'd (placeholder filled + `status: recorded` -> no transition, no auto-PR) — first live confirmation the scoping does nothing when it should. Durable defect record: PMN-018. Deferred PMN-018 residuals: directory-binding vs placeholder-coupling hardening; "PMN-001 (k)" label vs Observation-(k) collision; no script-test infrastructure.
- **(XII)** §23.6.5 behavioral-test record — three-instance enumeration correction + TASK-0048 design input. The committed PR-75 review-context records the gate-current status-pinning residual as "F-1 + P2" (two named) plus an unenumerated "sibling" allusion; the cycle actually exhibited THREE instances: (1) F-1 at §Current-state Summary (Codex pre-commit), (2) P2 at §Current-state file-list item 1 (Codex post-PR), (3) §4 L131 `greenfield.md` frontmatter pin (caught by P2's completeness grep at step-13). Recorded here for accurate TASK-0048 input (the review-context is append-only and undercounts). Result: the append-only suppression clause held — ZERO append-only-historical re-flags across both Codex phases (eliminated TASK-0045's claim-7/8/12 rewrite churn) — KEEP verbatim. For gate-current status-pinning the TASK-0048 §23.6.5 design input is NOT a completeness grep (a post-hoc guard) but the cross-Architect-ratified structural rule: volatile state has one current home; append-only surfaces may quote it only as an explicitly-labeled snapshot, else must point to the home. Volatile-field set to name: status, linked PR/SHA, branch/HEAD, last-completed-step, cumulative-diff-stats, artifact lifecycle state. Promote "the sole cycle-close marker is frontmatter/action-filled state, not body prose."
- **(XIII)** Cross-Architect review at cycle-close (ChatGPT) — ratified outcomes. Comprehensive state review conducted at close; primary-Architect adjudication owner-ratified. (a) Close = append-PR (this reconciliation) over PMN-018-only, framed brutally-narrow as durable-record reconciliation. (b) Carry-forwards VIII + X pulled into TASK-0047 as prompt-correctness dependencies (not deferred): the project-brief "Kickoff follow-ups" template gap (VIII) and usage-guide §2.1 trio under-spec vs greenfield Step 0 (X) both ground correct retrofit/upgrade prompts, and retrofit surfaces more follow-ups than greenfield; carry-forward V stays opportunistic / release-polish. (c) The live `linked-pr-fix-up` Action's regression safety is a v3.0 concern (not v3.1/Batch-P4) because it is operational today: add minimal script-level regression tests (handoffs->resolved; prompts stay; reviews/PMNs->recorded; templates untouched; linked_pr path-agnostic) before TASK-0047's first live run of the new scoping. (d) Narrow label-shape sweep at release polish (reclassify observation-letter-shaped labels as mechanism labels, or rename); not a broad citation audit. (e) Primary blind spot named = adopter-usability-under-first-contact: make TASK-0047 prompts runnable by a first-time operator and adopt an adopter-mode smoke test (run a prompt cold from README + canonical trio, record confusion points) as a standing prompt-cycle acceptance gate. Actioned items seed the TASK-0047 successor spec.
