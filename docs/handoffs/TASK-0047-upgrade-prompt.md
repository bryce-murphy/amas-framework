...
- **(IV)** **PR-B close + TASK-0047 FULLY COMPLETE.** All three Batch P3 kickoff prompts are now live on `main`: **greenfield** (PR-75 / v2.42), **retrofit** (PR-80 / v2.43), **upgrade** (PR-83 / v2.44). **Batch P3 is complete.** PR-A's close record is the PR-A handoff §10 (`docs/handoffs/TASK-0047-batch-p3-retrofit-upgrade-prompts.md`); this ledger closes the second (PR-B) half and thereby TASK-0047 as a whole.
- **(V)** **Convergence trail (PR-B), severity-monotonic to zero:** Mode-1 first-operator push-through smoke (**no hard walls / no forced fabrication** — the proven-baseline payoff: the scaffold walls were already fixed in PR-A) → Codex post-push (2 P2; absorbed `c0d55f2`) → targeted Codex re-pass (1 P2 parity + 1 P3 wording; absorbed `7cb9a2d`) → path-(α′) pre-merge status flip (`a77807c`). Far below PR-A's remediation load — the split-to-PR-B-on-the-proven-baseline decision paid off.
- **(VI)** **Central findings.** (a) **Upgrade-vs-bootstrap reframe**: an upgrade is a project-scope-shift **amendment**, not a fresh bootstrap (Step 5 re-validates/amends the existing set; Step 6 records an upgrade ADR + resumes; no Issue-0 / TASK-0000 / fresh bootstrap-set). ADR-009 plan-vs-instantiate applied to the **forthcoming migration mechanism** + the canonical **changelog/migration-notes surface** as non-blocking forthcoming. (b) **usage-guide §2 bootstrap-leak class-sweep + §2.1 claim-artifact-parity fix** — both caught by Codex's code-aware pass: distinct-defect-class evidence for the §24.5 multi-surface pipeline (Codex catches what self-review + scaffold-reuse don't, even on a proven baseline).
- **(VII)** **Action — second consecutive clean substitution-bearing run.** PR-84 linked-pr-fix-up substituted the squash SHA into the PR-B handoff `linked_pr` (`PR-83 (squash SHA 3606278)`) + flipped `status: active → resolved`, touching **only** the handoff; `prompts/` + `templates/` stayed untouched — **PMN-018 path-scoping held again** (the #76 over-reach guard confirmed a second time).
- **(VIII)** **§18.3 M-A7 34th-instance** landed at this close-reconciliation: count 33 → 34; span v2.16 → v2.44; `+ PR-83` appended; preamble + steady-state → v2.44 / PR-83 / TASK-0047. PR-84 (chore-fix-up) excluded per the substantive-cycle-only inclusion criterion. Resolves the (III) deferral (the v2.43 → v2.44 per-substantive-PR bump-call was Architect-confirmed at this close).
- **(IX)** **CARRY-FORWARDS to TASK-0048 (formal).** No PMN authored this cycle (trigger-based posture), so the four **monitoring-register candidates** ride forward — **comprehensive pre-merge red-team**; **class-sweep-not-instance-fix**; **gate-current-refresh-at-each-state-transition**; **claim-artifact-parity** — for registration at TASK-0048's PMN, with PR-A §10 (VIII) + this cycle's evidence (red-team caught 5/6 preventable at PR-A; class-sweep closed the PR-0-sentinel + usage-guide-leak re-raises; gate-current drift took 3 reconciliation passes at PR-A; claim-artifact-parity caught the phantom review-context + the §2.1 upgrade-handoff over-claim). The **canonical changelog/migration-notes surface** ((II)) carries forward as a companion to the forthcoming migration mechanism (post-v3.0). Next: Part C.2 (TASK-0048) → v3.0.0 (TASK-0049).
