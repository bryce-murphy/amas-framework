...
- **Step 15.4 fix-up commit #2 `6d6f3b7` + push**: complete.
- **Step 16 Codex post-PR pass-3 (UTC 2026-05-07T11:07:28Z, reviewed `6d6f3b76c2`)**: complete; verdict Major × 1 (P2 review-context Claim 16 PR-35 sweep ADR-006 false-absence claim = 5th-instance (h.2) at different location).
- **Step 16.1 Architect step-13-pass-3 re-assessment**: complete per Architect step-13-pass-3 §1-§5 ratification — bounded-continuation rule continues to apply (per-pass rate trajectory 2→1→1→1 decreasing; not genuinely-asymptotic per `core.md` §8.1.1.3); sub-cluster diagnosis identified Builder-claims-to-verify discipline gap (3-of-5 instances); Option (iii) ratified (apply R.8 + queue lightweight canonical absorption for TASK-0028+).
- **Step 16.2 Edit R.8 application**: complete.
- **Step 16.3 §23.6.2 self-review pass-5**: complete; convergence at iteration 1; no same-class recurrence at R.8 site.
- **Step 16.4 fix-up commit #3 + push**: in progress.
- **Step 17 Architect §24.3.1 five-point post-handback check**: pending owner direction on pass-5 invocation vs hand-back; Architect lean: hand-back per Architect step-13-pass-3 §6 calibrated marginal-yield assessment.
...
**Cumulative (h.2) intra-cycle count = 5** (R.1.1 + R.1.2 + R.3 + R.5 + R.6 + R.8) with sub-cluster decomposition: Builder-claims-to-verify (R.3 + R.5 + R.8 — 3 instances) + template-byte-exact-regex (R.1.1 + R.6 recursive chain — 2 instances). Per-pass rate 2→1→1→1 decreasing; NOT genuinely-asymptotic per `core.md` §8.1.1.3. (h.2) sub-shape canonical at PMN-009 already; cycle's empirical record strengthens existing canonical, not new sub-shape. **(k.1) salience MAXIMUM** per PMN-008 §3.1 framework (alongside TASK-0023 / TASK-0025 priors).

**Pass-3** (UTC 2026-05-07T11:07:28Z, reviewed `6d6f3b76c2`): Major × 1 (P2 review-context Claim 16 PR-35 sweep false `ADR-006 not present` claim — 5th-instance (h.2) at different location triggering re-assessment per Architect step-13-pass-2 §7). Architect step-13-pass-3 §1-§5 re-assessment: bounded-continuation rule continues to apply per `core.md` §8.1.1.3 cost-class refinement (per-pass rate decreasing); sub-cluster diagnosis (Builder-claims-to-verify forward-looking claims authored as anticipated-prose vs verify-at-authoring-output); Option (iii) ratified — apply Edit R.8 path-(a) + queue lightweight canonical absorption for TASK-0028+ Builder-claims-to-verify sub-cluster sub-rule (Item 12 NEW). Adjudicated path-(a); Edit R.8 applied (review-context Claim 16 false `ADR-006 not present` parenthetical → empirically-correct 7-site enumeration; intra-document Claim-16-vs-step-10-table consistency restored). §23.6.2 self-review pass-5 iteration-1 converged.

Cycle-close ledger Items 1 + 9 + 12 + 13 amendments / additions + new Items 10 + 11 per cumulative Architect step-13-pass-2 + step-13-pass-3 ratifications.
...
- **Item 9 — (h.2) intra-cycle recurrence empirical observation — 5-instance count with sub-cluster decomposition + recursive (k.1) MAXIMUM at R.1.1 → R.6** (amended at Codex post-PR pass-3 absorption per Architect step-13-pass-3 §4 ratification): **5-instance** (h.2) verification-command operational correctness cluster within TASK-0027 cycle with **sub-cluster decomposition**:
  - **Builder-claims-to-verify sub-cluster (3 instances)**: Codex pre-commit Finding 3 (R.3 review-context Claim 10 PR-12 count `2` → `7`) + Codex post-PR pass-1 (R.5 review-context Claim 5 fourth bullet ERE escaping → `grep -F`) + Codex post-PR pass-3 (R.8 review-context Claim 16 false `ADR-006 not present` → empirically-correct 7-site enumeration). Common authoring mechanism: claims authored as anticipated-prose rather than verify-at-authoring-output.
  - **Template-byte-exact-regex sub-cluster (2 instances; recursive chain)**: Codex pre-commit Finding 1 (R.1.1 + R.1.2 handoff-template `linked_pr` exemplar `PR-XX` → `PR-N`) + **Codex post-PR pass-2 (R.6 handoff-template:21 inline YAML annotation strip — recursive defect introduced by R.1.1 itself; structurally definitive resolution eliminates recurrence root)**.
  
  **Per-pass rate trajectory: 2 → 1 → 1 → 1 (decreasing)** per Architect step-13-pass-3 §2 cost-class analysis. NOT genuinely-asymptotic per `core.md` §8.1.1.3; high-density-of-claim-surfaces cycle is the driver, not asymptotic loop. Each individual finding remains pure-token-swap one-iteration; no cascade at fix-location. (h.2) is already canonical at PMN-009 sub-shape framework; intra-cycle recurrence is empirical confirmation of existing canonical, not new defect class.
  
  **Architect step-back diagnosis closed-the-loop in-cycle** per Architect step-13-pass-2 §2 (R.6 structural reframe at template surface) + Architect step-13-pass-3 §3 (Builder-claims-to-verify sub-cluster diagnosis). Bounded-continuation rule + cost-class refinement validated itself empirically across both diagnoses. Empirically strengthens (k.1) MAXIMUM framing per Item 1 amendment — multi-surface review pipeline catches (h.2) drift at distinct surfaces, individually one-iteration-resolved; no PMN-promotion needed for already-canonical sub-shape. **Lightweight canonical absorption candidate at TASK-0028+ for Builder-claims-to-verify sub-cluster sub-rule per Item 12 NEW.**
...
- **Item 12 NEW — Builder-claims-to-verify authoring discipline lightweight canonical absorption candidate** (added at Codex post-PR pass-3 absorption per Architect step-13-pass-3 §3 + §4 ratification): 3-of-5 (h.2) intra-cycle instances cluster at "Builder-claims-to-verify forward-looking claims" sub-shape (R.3 + R.5 + R.8) — common authoring mechanism: claims authored as anticipated-prose rather than verify-at-authoring-output. Builder writes prose enumerating expected output without running the verification command at authoring time AND incorporating actual output verbatim. PMN-009 (h.2) sub-shape canonical mechanism applies recursively to claim-authoring surface itself (when the verification surface — review-context Claims — is itself authored under sub-shape A discipline, sub-shape A applies recursively to the claim authoring per `core.md` §23.6.3 sub-shape A). **Lightweight canonical absorption candidate at TASK-0028+**: 1-2 line addition to PMN-007 §2.4 (j)-sweep discipline OR `core.md` §23.6.3 sub-shape A enumeration codifying: "Builder-side verification-claim authoring sub-rule: When authoring review-context Claims-to-verify with bash/PowerShell verification commands, run the command at authoring time against the staged-tree state and incorporate actual output verbatim into claim prose. Anticipated-prose claims (authored without running the command) systematically produce (h.2) operational-correctness drift; PMN-009 (h.2) sub-shape canonical mechanism applies recursively to claim-authoring surface itself." Carry-forward to TASK-0028+ adjudication; could ride along with Item 10 template-content authoring sub-rule + Item 11 (j)-sweep enumeration completeness sub-rule as bundled lightweight canonical amendment cycle. NOT PMN-011 per ADR-006 D3 evidence-bar discipline cost-benefit framework.

- **Item 13 NEW — Calibration update on Architect Option A vs Option B framing** (added at Architect step-13-pass-3 §5 ratification): Architect's step-13 pass-2 §2 recommendation favored Option B (skip pass-3 + hand back) on "expected marginal yield is low" reasoning. Empirical update: owner's Option A direction caught a real (h.2) defect at pass-3 (Edit R.8). Architect probability estimate was on the lower end of marginal yield expectation but not zero; pass-3 yield validates Option A choice empirically. **Calibration update for forward decisions**: in cycles with high-density verification-claim surfaces + recursive (k.1) MAXIMUM salience, pass-marginal-yield estimates should bias higher than a default cycle. The empirical record from this cycle (pass-3 caught Claim 16 false-absence; pass-2 caught recursive R.6) suggests Codex multi-pass thoroughness has higher floor for high-density-claim cycles than prior estimate. Single-cycle observation on Architect-side estimation calibration; carry-forward for future high-density-claim cycle adjudications. NOT PMN-011 per ADR-006 D3.
