...
*[NOT in PR-A — deferred to PR-B: `prompts/upgrade.md` (stays stub). Pre-commit-pending per §Last-completed-step open items: §18.3 M-A7 increment; `docs/reviews/PR-80-codex-pre-commit.md` review-context (authored at the commit/PR-open step).]*

**Cumulative-diff-stats** *(gate-current; assembly pre-commit working-tree state — not yet staged)*: 9 modified tracked files = +422 insertions / −54 deletions vs `origin/main` (`git diff --shortstat origin/main`); plus 2 new files — `ADR-009` (73 lines) + this handoff (line count shifts with this reconciliation). 11 files total. (XVII) bidirectional sum-stability + exact final staged numbers are verified at Gate A after `git add` per `core.md` §23.6.1.1 (e.1) staged-tree convention (the untracked-file lines are not in `git diff` until staged).
...
Assembly content pass done (v2.43 dogfood bump ×3 surfaces + forthcoming-qualifier sweep + README retrofit Prompts-row + this gate-current reconciliation). Next = Architect confirms the canonical specifics (§18.3 M-A7 33rd-instance decision; handoff `status: drafted → active` token-swap; the prompts' 3d "reference v2.42" quote vs the v2.43 bump) → commit + push + open draft PR-A → Codex GitHub-App branch review → absorb (path-a) / defer (path-β) → Gate A/B → owner squash-merge → chore-fix-up watch → cycle-close ledger. `prompts/upgrade.md` ships as PR-B on the merged baseline.
...
- *[Cycle-close observations + new PMN candidates append at cycle close.]*
