...
PR-80 red-team batch fix committed + pushed (draft open). Next = targeted red-team re-verification on the fixed branch (re-check the 6 finding-classes + the convergence-sweep classes) → on a clean re-pass, Gate A + Gate B closure (§24.3.1 XXVI two-gate) → mark PR-80 ready → owner squash-merge → linked-pr-fix-up Action watch (first substitution-bearing run on `docs/handoffs/` + `docs/reviews/`: expect handoff `linked_pr` SHA-substitution + `status: active → resolved`, and review-context `status: drafted → recorded`) → cycle-close ledger + deferred `core.md` §18.3 M-A7 33rd-instance post-merge increment (verified PR-80). `prompts/upgrade.md` ships as PR-B on the merged baseline.
