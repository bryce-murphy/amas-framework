...
Step 16.1 Codex post-PR pass-3 absorption + R.18 path-(a) within-surface sub-component-enumeration-directive fix-up in-progress under Architect's Option C ratification 2026-05-07. Codex post-PR pass-3 absorbed verbatim into review-context per PMN-002 (a) + `core.md` §8.1.1.2; verdict 0 Blocking / 1 Major P2 / 0 Minor — NOT cycle-trailing-clean Approve (5th-instance Item 11 (h.2) cluster at within-surface absorption-completeness sub-class adjacent to surface-enumeration scope). Architect step-15 directive overridden per refined diagnostic: pass-4 self-review converged clean at MAXIMAL surface-enumeration scope (zero findings, zero new-class within enumeration); pass-3 finding is at NEW sub-class adjacent to surface-enumeration (within-surface absorption-completeness; coupled value + SHA/timing-qualifier shape NOT in enumeration scope). Item 11 load-bearing-with-maximal-application property VALIDATED at surface-enumeration scope, not refuted; pass-3 finding is discovery of ADJACENT discipline gap. Option C ratified: path-(a) R.18 with within-surface sub-component-enumeration directive + bounded commitment (ANY pass-4 finding → immediate Option B fallback). Iteration toward step-14.9 §23.6.2 self-review pass-5 with within-surface sub-component-enumeration completeness verification + step-14.10 fix-up commit + push + step-17 Codex post-PR pass-4 cycle-trailing-clean confirmation per `core.md` §8.1.1.1. Pass-4 outcome determines Item 11 + new within-surface sub-discipline empirical validation per Architect Option C directive.

## Current state

**Summary**: 8 files modified on branch `chore/task-0028-bundled-lightweight-absorption-m-a7` post-step-12 commit `c72a727` + step-13 push + step-14.3 fix-up commit `eb7f7ed` + push + step-14.7 fix-up commit `741005d705a445e8cccd3bd90ffb06755e80a5fd` + push (current PR-37 head). PR-37 OPEN at [https://github.com/bryce-murphy/amas-framework/pull/37](https://github.com/bryce-murphy/amas-framework/pull/37); mergeable=MERGEABLE; mergeStateStatus=BLOCKED per ADR-001 D9 admin-bypass posture pre-merge. Codex post-PR pass-3 absorbed (1 P2 Major finding at within-surface absorption-completeness sub-class; non-cycle-trailing-clean); R.18 within-surface sub-component-enumeration-directive fix-up staging in progress per Architect Option C ratification. Cumulative-diff-stats per (e.1) re-derived post-R.18 staging vs origin/main (cycle-level cumulative; step-14.10 fix-up commit will surface delta-from-741005d separately).
...
**Cumulative-diff-stats** (per `core.md` §23.6.1.1 (e.1) staged-tree convention; landed-exact cumulative-from-origin/main at HEAD 741005d post-step-14.7-fix-up-commit + R.18 within-surface sub-component-enumeration-directive staging; re-derived per Architect step-14.3 §24.3.1 cross-check Item 4 + step-15 MAXIMAL-enumeration-directive Item H6 + step-16 Option C within-surface sub-component-enumeration directive): `8 files changed, 618 insertions(+), 6 deletions(-)`. Spec §1.1 anticipation ~261-369 ins / ~3 del across ~8 files; cumulative del 6 stable from step-11.1 (fix-up del-from-prior-commits targeted Builder-authored content cancelling at cumulative-from-origin/main level). R.6-R.17 fix-up delta landed at step-14.7 commit 741005d (118+/17− vs eb7f7ed); R.18 within-surface sub-component-enumeration delta surfaces at step-14.10 commit per pass-5 self-review verification. Order of magnitude smaller than TASK-0027 (1038/58/10) per ADR-006 D3 lightweight-absorption efficiency despite recursive absorption-iteration overhead at MAXIMUM-salience cycle.
...
- **Step 17 Codex post-PR pass-4 cycle-trailing-clean confirmation OR Option B fallback**: pending per `core.md` §8.1.1.1; owner re-triggers `@codex review` post-R.18 push; pass-4 outcome routes per Architect Option C bounded-commitment: clean Approve → cycle ships + Items 11 + new within-surface sub-discipline empirically validated; ANY finding (regardless of class) → Option B fallback per bounded commitment + cycle ships with carry-forward + TASK-0029 immediate structural reframe scope expansion (Items 7 + 14 bundled).
- **Step 18 hand-back to Architect**: pending; §24.3.1 five-point check by Architect.
...
**Item 8 — MAXIMAL enumeration directive empirical record at TASK-0028**: maximal upfront enumeration discipline application converges at 1 iteration vs reactive-per-finding 4+ iterations. Pass-4 zero-finding + zero new-class within enumeration scope. Preliminary positive evidence FOR Item 11 sub-rule's load-bearing-with-maximal-application property at surface-enumeration scope. Pass-3 surfaced ADJACENT sub-class finding (not refutation; see Item 11 below for refined diagnostic). Surfaced at Architect step-14.6 §24.3.1 cross-check.
...
**Item 10 — MC-A diff-stats pattern refinement (5th-cycle data point with new signal)**: MAXIMUM-salience cycles systematically overshoot default-cycle anticipation tolerance bounds via recursive absorption-iteration overhead. TASK-0027 + TASK-0028 both overshot; TASK-0028 more pronounced (cumulative ins +47.7% above spec §1.1 upper bound; +23.1% above ±20% MC-A tolerance). Refinement candidate: "MAXIMUM-salience cycles bias anticipation upper bound +30-50% above default-cycle baseline." Forward review at TASK-0029 Phase 1. Surfaced at Architect step-14.6 §24.3.1 cross-check.

**Item 11 — Item 11 load-bearing-at-MAXIMAL-application property (refined diagnostic)**: VALIDATED at surface-enumeration scope (pass-4 clean); ADJACENT discipline gap discovered at within-surface sub-component scope (pass-3 finding). Item 11 sub-rule + new within-surface sub-component-enumeration sub-discipline are separable disciplines; surface-enumeration alone insufficient when coupled-shape sub-components present. Pass-3 finding is sub-class discovery, NOT structural refutation. Step-15 binary-outcome routing override correctly applied. Surfaced at Architect step-16.2 stop-and-show.
...
**Item 14 — Within-surface sub-component-enumeration sub-discipline candidate**: sub-shape candidate for Item 11 future refinement at TASK-0029+ — surface-enumeration MUST be paired with within-surface sub-component-enumeration when sub-components are coupled (numeric-value + contextualizing-qualifier shape; field + field-comment shape; step-status + step-log-event shape; etc.). Item 11 sub-rule sufficient when applied at full granularity (surface + within-surface sub-component); reactive per-finding application empirically inadequate; surface-level upfront enumeration improves but doesn't fully eliminate. NOT PMN-eligible per ADR-006 D3 (single-cycle observation); structural-reframe-candidate alongside Item 7. Surfaced at Architect step-16.2 stop-and-show.
