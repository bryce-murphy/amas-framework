...
Authoring + sweep + standard surfaces + artifacts done; §5 battery 10/10 PASS; Gate A cleared (twice — initial + post pre-commit-absorption). Codex pre-commit/red-team absorbed (3 path-(a): F1/F2/F3); committed `9098bd2`, pushed, PR #86 opened (PR-open re-verification: actual #86 = anticipated). Codex post-PR passes 1–8 absorbed and pushed: P1 §13.1 ratifications; P2 gate-current refresh; pass-3/4 self-volatile-pinned-total class (gate-current pointers, then de-pin); **pass-5 P2 = §23.6.5 by-pointer clarification; pass-6 = stale-head path-(β); pass-7 P2 = §13.1 parallel by-pointer clarification (by-pointer class then exhausted, 2 members); pass-8 P2 = §8.2 item-(4) pre-branch-timing fix.** Pushed through `eb09645` (pass-9: §8.2/§10.5/§8.3 cross-surface fixes). **Pass-10 (usage-guide §3.9 + one-page bypass-rule rewording → canonical "optional" posture; bypass-ack class swept clean across all 4 surfaces) absorbed, Gate A re-cleared, ratified, and pushed.** Canonical figure now **+189/−62** (core.md 141/12; usage-guide 15/15, deltas explained at §Cumulative-diff-stats).

## Current state Summary  *(GATE-CURRENT — post-PR / Gate-B-approach state)*

Five Part C.2 members materialized in `core.md` in document order (§8.2 → §8.3 → §10.5 → §13/§13.1/§13.2 → §23.6.5), each schema-grade. Class-bound sweep complete: in-scope qualifier residue = 0; STAY-13 byte-unchanged; D2 migration materialized-only (§2.3.6 retained as substrate); PR-template canonical converged to live attestation form with `.github` mirror byte-unchanged; §10.5 → `github-reference.md` §3.2 cross-ref present with provenance (:148/:379) untouched; L546 + the two ruled Item-14 migrations applied. Class A v2.45 across 4 sites (zero v2.44/v2.43 residual); §18.3 M-A7 35th internally consistent across 5 sub-claims; README roadmap rotated (Part C.2 shipped). Zero open same-cycle forward-refs. PR #86 open; Codex pre-commit (3) + post-PR passes 1–10 absorbed + pushed; **pass-10 (usage-guide §3.9 + one-page bypass-rule rewording → canonical "optional" posture; bypass-ack class swept clean across all 4 surfaces) Gate-A-re-cleared, ratified, and pushed.** Canonical figure +189/−62 (core.md 141/12; usage-guide 15/15). Awaiting binding Codex re-review of the latest pushed HEAD → Gate B.

## Cumulative-diff-stats  *(GATE-CURRENT — post-PR / Gate-B-approach state; re-derive (e.1) at each staged-tree mutation)*

- **Canonical + operational surfaces**: 10 files, **+189 / −62** (net +127). Authoring (C1–C4) = +128 pure-add; sweep/migration/standard + post-PR canonical fixes = the remainder. Per-file: core.md 141/12 (the +1 vs the prior +187 baseline = the §8.3 `gh pr edit` bullet, pass-9); usage-guide 15/15 (the +1 vs the prior 14/14 = the pass-10 one-page bypass-rule rewording, L402, completing the §10.5 acknowledgment reconciliation on the usage-guide side); AGENTS 6/6; CLAUDE 6/6; README 2/2; templates/AGENTS 5/5; templates/CLAUDE 5/5; templates/handoff 4/4; templates/PULL_REQUEST_TEMPLATE 4/6; templates/role-scorecard 1/1.
- **Cycle artifacts (this pass)**: + PMN-019, + this handoff, + review-context `PR-86-codex-pre-commit.md` (3 new files).
- **Full staged total (incl. the 3 self-volatile cycle artifacts)**: **by pointer** — see the PR #86 diff (`git diff main --shortstat`); **not pinned here**, per §23.6.5 reference-volatile-by-pointer (the full total moves on every docs-only fix-up, so no surface pins it — this structurally ends the self-volatile-pinned-total recurrence). The stable substantive figure is the canonical **+187/−61** above. github-reference.md + `.github/PULL_REQUEST_TEMPLATE.md` byte-unchanged (0/0).
...
- **Stop-and-show points reached (§8.3) — payload + owner ratification pairs** (per §13.1, each record carries the §8.3 payload AND the owner ratification that followed):
  ...
  - Codex post-PR pass-10 absorption: payload = P2 **canonical-law** reverse-direction completion of the §10.5 reconciliation — usage-guide §3.9 (L118) + one-page bypass rule (L402) reworded from mandatory "with explicit acknowledgment" to the canonical "optional" posture; bypass-ack class swept clean (optional across all 4 surfaces, zero residual); Gate A re-cleared → **owner ratified** (fix-up push granted).
