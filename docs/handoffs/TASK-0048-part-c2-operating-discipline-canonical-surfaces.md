...
Authoring + sweep + standard surfaces + artifacts done; §5 battery 10/10 PASS; Gate A cleared (twice — initial + post pre-commit-absorption). Codex pre-commit/red-team absorbed (3 path-(a): F1/F2/F3); committed `9098bd2`, pushed, PR #86 opened (PR-open re-verification: actual #86 = anticipated). Codex post-PR passes 1–7 absorbed and pushed: P1 §13.1 ratification records; P2 gate-current refresh; pass-3 P2 gate-current pointer class-sweep; pass-4 P2 self-volatile-pinned-total de-pin; **pass-5 P2 = §23.6.5 canonical by-pointer clarification (current/live, historical snapshot carveout); pass-6 = stale-head path-(β); pass-7 P2 = §13.1 parallel by-pointer clarification (same class as pass-5; §8.3 payload historical snapshots carved out). Canonical figure +187/−61 unchanged (both pass-5 + pass-7 are in-place edits). Fix-up ratified + pushed to PR #86.**

## Current state Summary  *(GATE-CURRENT — post-PR / Gate-B-approach state)*

Five Part C.2 members materialized in `core.md` in document order (§8.2 → §8.3 → §10.5 → §13/§13.1/§13.2 → §23.6.5), each schema-grade. Class-bound sweep complete: in-scope qualifier residue = 0; STAY-13 byte-unchanged; D2 migration materialized-only (§2.3.6 retained as substrate); PR-template canonical converged to live attestation form with `.github` mirror byte-unchanged; §10.5 → `github-reference.md` §3.2 cross-ref present with provenance (:148/:379) untouched; L546 + the two ruled Item-14 migrations applied. Class A v2.45 across 4 sites (zero v2.44/v2.43 residual); §18.3 M-A7 35th internally consistent across 5 sub-claims; README roadmap rotated (Part C.2 shipped). Zero open same-cycle forward-refs. PR #86 open; Codex pre-commit (3) + post-PR passes 1–7 absorbed + pushed (passes 3–4 = self-volatile-pinned-total class; pass-5 = §23.6.5 by-pointer clarification; pass-6 = stale-head path-(β); pass-7 = §13.1 parallel by-pointer clarification, same class as pass-5). Canonical figure +187/−61 unchanged throughout. Awaiting binding Codex re-review → Gate B.
...
- **Stop-and-show points reached (§8.3) — payload + owner ratification pairs** (per §13.1, each record carries the §8.3 payload AND the owner ratification that followed):
  ...
  - Codex post-PR pass-7 absorption: payload = P2 **canonical-law** fix — narrowed core.md §13.1 by-pointer rule to current/live state (parallel to §23.6.5 pass-5 fix; historical §8.3 payload snapshots carved out) → **owner ratified** (fix-up push granted).
