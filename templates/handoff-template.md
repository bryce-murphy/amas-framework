...
- **Body `- Status:` vs frontmatter `status:`**: the `## Metadata` body `- Status:` field is a last-gate snapshot the Builder writes at hand-back; it is NOT maintained by the linked-pr-fix-up Action, which transitions only the frontmatter `status:` field per PMN-001 (k). Do not expect the body `- Status:` to auto-update at merge — it reflects the gate state at which it was written. (Per `core.md` §13.2.)
