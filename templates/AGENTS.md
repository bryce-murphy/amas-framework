...
Canonical receiving-surface instruction file for Codex AI agents (OpenAI Codex desktop, Codex CLI, successor Codex products) operating at an AMAS-adopted repository. At an adopter project, this template is materialized as `AGENTS.md` at repository root and declares receiving-surface identity, operational expectations, and project-specific overlay. AMAS canonical discipline applies identically across AI receiving surfaces; receiving-surface-specific operational depth lives in receiving-surface adapter packs at `appendices/receiving-surface-adapters/codex.md` per Batch P7 (ship-pending per ADR-006 D2 + ADR-007 batch sequence).

## §1. Receiving-surface identity

This file targets Codex AI agents at `<PROJECT NAME>`. Receiving-surface identity is fixed at this file; sibling receiving-surface canonical instruction files address other AI families:

- `CLAUDE.md` — Anthropic Claude products (Claude.ai Project, Claude Code CLI, Claude desktop, successor Claude products)
...
2. This file (`AGENTS.md`) — receiving-surface identity + operational expectations
...
Receiving-surface-specific operational depth lives at `appendices/receiving-surface-adapters/codex.md` per Batch P7 (ship-pending).
...
Codex desktop pre-commit invocation + post-PR `@codex review` invocation are owner-driven per `core.md` §8.1.1.1 + ADR-001 D11 owner-invokes convention.
...
- Receiving-surface adapter pack for Codex: `appendices/receiving-surface-adapters/codex.md` (Batch P7; ship-pending)
- Sibling receiving-surface canonical instruction file: `CLAUDE.md` (Anthropic Claude products)
