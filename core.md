...
**Empirical grounding (cross-cycle accumulation).** Cost-class one-iteration convergence at pure-token-swap class has accumulated 12 cross-cycle empirical positives across TASK-0019 through TASK-0034 cycles: every cycle's Codex pre-commit + post-PR absorption iterations applying path-(α')/path-(α)/path-(a) class fixes have converged at one iteration; zero cross-cycle empirical negatives surface. Codex pass-2 re-invocation is therefore NOT REQUIRED at pure-token-swap class absorption per §8.1.1.3 cost-class refinement; owner-ratification gate at re-derived (e.1) post-fix surface sufficient. Pattern is strongly canonicalized beyond ADR-006 D3 evidence-bar (3+ confirmations) by factor 4×.
...
**Cumulative empirical instances post-v2.16 canonicalization** (as of v2.31 canonicalization at PR-52 / TASK-0035):

The original four-instance empirical grounding above documented the M-A7 promotion event at PR-13 / v2.16. Subsequent substantive-cycle PRs have continued instantiating the M-A7 pattern at consistent cadence. Cumulative count per established enumeration `PR-9 + PR-10 + PR-11 + PR-13 + PR-15 + PR-17 + PR-19 + PR-21 + PR-25 + PR-27 + PR-29 + PR-31 + PR-33 + PR-35 + PR-37 + PR-39 + PR-41 + PR-43 + PR-45 + PR-48 = 20` empirical instances spanning v2.16 through v2.30 canonicalization (substantive-cycle PRs only; defect-fix patches and chore-fix-up substitution PRs excluded per established M-A7 inclusion criterion).

The pattern has stabilized into operational steady-state across 20 consecutive substantive cycles; further cumulative-count amendments occur as Architect-side post-merge maintenance per established M-A7 cadence.
...
The check applies at all staged-tree-mutating actions, including absorption operations. Codex pass absorption itself mutates the staged tree; re-derivation post-absorption is mandatory, not optional. Empirical grounding: 5+ in-cycle positives across TASK-0030 through TASK-0034 cycles; canonical-text amendment cycle (TASK-0035) materializes refinement candidate (XVII) per ADR-006 D3 evidence-bar.
...
**Two-gate Gate A + Gate B discipline.** The five-point post-handback check applies at two distinct gate surfaces in the staged-tree → origin/feat → merge progression:

- **Gate A — staged-tree pre-commit state.** Architect §24.3.1 five-point check applied against Builder hand-back claims at staged-tree state pre-commit. SHA verification at Point 2 anchored to staged-tree HEAD; file-content audit at Point 3 anchored to staged-tree state; phantom-action audit at Point 4 anchored to commit log if commits exist (may be empty pre-commit). Gate A surface: post-step-12.X (e.1) re-derivation hand-back; pre-step-13 commit.
- **Gate B — origin/feat empirical attestation post-commit+push pre-merge state.** Builder origin/feat empirical attestation against remote-state at post-commit+push pre-merge. Three-surface SHA concordance check: local HEAD = origin/feat HEAD = PR-NN headRefOid. Bidirectional sum-stability concordant at 3 axes per (XVII). Gate B surface: post-step-15 post-PR Codex pass-1 absorption; pre-step-17 owner merge.

Both gates required for cycle-close ratification. Gate A PASS at staged-tree state does not substitute for Gate B at origin/feat state (commit + push intervenes between gates; pre-merge state may diverge from staged-tree state if intervening Builder action lands). Empirical grounding: 3 cross-cycle instances (TASK-0032 1st-instance recursive-self-instantiation at chore-fix-up; TASK-0033 2nd-instance test; TASK-0034 3rd-instance PASS at adoption-pilot cycle); ADR-006 D3 evidence-bar met at TASK-0034 cycle close per cycle-close ledger (XXVI) entry.
