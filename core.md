...
4. **Branch-name conformance** — at **pre-branch** pre-flight, the **proposed** task branch name (the one about to be created) matches the `github-reference.md` §2.2 `<type>/task-####-<kebab-slug>` regex (canonical at v3 per ADR-005); **once the branch exists**, the actual working branch is verified against it. At pre-branch time the working branch is still the base branch (typically `main`), so this item validates the proposed name — not the current working branch, which would wrongly FAIL a conforming pre-branch pre-flight.
...
**Cumulative empirical instances post-v2.16 canonicalization** (as of v3.0.0 canonicalization at PR-89 / TASK-0049):

The original four-instance empirical grounding above documented the M-A7 promotion event at PR-13 / v2.16. Subsequent substantive-cycle PRs have continued instantiating the M-A7 pattern at consistent cadence. Cumulative count per established enumeration `PR-9 + PR-10 + PR-11 + PR-13 + PR-15 + PR-17 + PR-19 + PR-21 + PR-25 + PR-27 + PR-29 + PR-31 + PR-33 + PR-35 + PR-37 + PR-39 + PR-41 + PR-43 + PR-45 + PR-48 + PR-52 + PR-54 + PR-56 + PR-58 + PR-60 + PR-62 + PR-64 + PR-66 + PR-68 + PR-71 + PR-73 + PR-75 + PR-80 + PR-83 + PR-86 + PR-89 = 36` empirical instances spanning v2.16 through v3.0.0 canonicalization (substantive-cycle PRs only; defect-fix patches and chore-fix-up substitution PRs excluded per established M-A7 inclusion criterion).

The pattern has stabilized into operational steady-state across 36 consecutive substantive cycles; further cumulative-count amendments occur as Architect-side post-merge maintenance per established M-A7 cadence.
...
**Major bump (vN → v(N+1)).** Major bumps restructure canonical patterns, introduce breaking changes to artifact forms, or cascade superseded ADR effects across multiple sections. Major bumps may require adopters to migrate. Empirically grounded examples: v3.0.0 (full v3 framework package release per the ADR-008-amended v3.0 scope — the minimum-viable canonical framework; transitioning from v2.x in-canonical-document framework to v3 distributed package shape).
...
- **Append-only historical surfaces** — the handoff's §3 step-by-step execution record, the pre-commit absorption section, the post-PR review section, and the handoff's §10 cycle-close ledger. These are **never back-refreshed**: a session appends, it does not rewrite prior entries. They reference **current/live** volatile state by pointer (e.g. "see §Cumulative-diff-stats") rather than copying a live value that would later read as stale. A value pinned explicitly as a **historical snapshot** — what was true when the entry was written, including quoted prior diff totals or verbatim reviewer output — is correct-by-design per the suppression clause below, not a violation of this by-pointer rule (the rule governs live-state references, not historical records).
