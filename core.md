...
**Two-gate Gate A + Gate B discipline.** The five-point post-handback check applies at two distinct gate surfaces in the staged-tree → origin/<branch> → merge progression (where `<branch>` = cycle's branch name per `github-reference.md` §2.2 + ADR-005 branch type variability — `feat|fix|chore|adr|shadow|spike`):
...
- **Gate B — origin/<branch> empirical attestation post-commit+push pre-merge state.** Builder origin/<branch> empirical attestation against remote-state at post-commit+push pre-merge. Three-surface SHA concordance check: local HEAD = origin/<branch> HEAD = PR-NN headRefOid (where `<branch>` = cycle's branch name per `github-reference.md` §2.2 + ADR-005 branch type variability). Bidirectional sum-stability concordant at 3 axes per (XVII). Gate B surface: post-step-15 post-PR Codex pass-1 absorption; pre-step-17 owner merge.

Both gates required for cycle-close ratification. Gate A PASS at staged-tree state does not substitute for Gate B at origin/<branch> state (commit + push intervenes between gates; pre-merge state may diverge from staged-tree state if intervening Builder action lands). Empirical grounding: 3 cross-cycle instances (TASK-0032 1st-instance recursive-self-instantiation at chore-fix-up; TASK-0033 2nd-instance test; TASK-0034 3rd-instance PASS at adoption-pilot cycle); ADR-006 D3 evidence-bar met at TASK-0034 cycle close per cycle-close ledger (XXVI) entry.
